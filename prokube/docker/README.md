...
This directory contains the Dockerfile and configuration for building a Kubeflow-compatible notebook image that runs the **prefix-aware OpenCode Web UI** (`prokube/app-prefixable`).
...
docker build \
  ...
  -t pk-opencode-prefixable:latest \
...
```bash
# Without base path
docker run -p 8888:8888 pk-opencode-prefixable

# With base path (simulating Kubeflow)
docker run -p 8888:8888 -e NB_PREFIX=/notebook/user/opencode/ pk-opencode-prefixable
```
...
spec:
  template:
    spec:
      containers:
        - name: opencode
          image: your-registry/pk-opencode-prefixable:latest
          ports:
            - containerPort: 8888
              name: notebook-port
          volumeMounts:
...
| `KF_EXAMPLES_REPO` | (see Dockerfile)        | Git repo to clone on startup      |
...
│   └── 02-clone-examples   # Clone kubeflow-examples (optional)
└── services.d/
    └── opencode/
        └── run             # Start API + UI servers
```

## Differences from opencode-notebook

| Feature         | opencode-notebook          | opencode-prefixable     |
| --------------- | -------------------------- | ----------------------- |
| UI              | Upstream (app.opencode.ai) | Custom (app-prefixable) |
| Prefix Handling | Server-side rewrite        | Native runtime support  |
| Build           | Uses upstream release      | Builds UI from source   |
...
1. Make changes in `prokube/app-prefixable/`
